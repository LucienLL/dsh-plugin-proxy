...
3. **Publish npm** — ✅ `dsh-plugin-proxy@0.1.0`（2026-08-21）与 **`0.2.0`（2026-08-22，系统代理轮询跟随 + ProxyOverride 合并）** 均已上线。方式：官方 npm CLI 安全密钥流程（`scripts/publish-interactive.ps1` → 浏览器打开 auth URL → 通行密钥确认），npm 2FA 自 2026 起为 WebAuthn-only、无 TOTP 验证码；完整经验见 `npm-publish` 技能。
