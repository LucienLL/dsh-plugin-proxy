...
3. **Publish npm** — `pwsh scripts/publish-npm.ps1` (token: `NPM_PUBLISH_TOKEN` env var at process or User scope, then `NPM_TOKEN`, then `$DSH_HOME/secrets/npm-token.txt`). ⏳ awaiting `setx NPM_PUBLISH_TOKEN`.
